...
public class FilterCatalogController implements InitializableController {
    ...
    public void setMainApp(Application app, UserSession userSession) {
        ...
        if (userSession.getToken() != null) {
            ...
            System.out.println("Error: Token is null, cannot initialize categories, products, or companies.");
        }
    }
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        System.out.println("Initial profile button text set to: Имя");
    ...
    private void updateBuyerInfo() {
        if (userSession == null || userSession.getToken() == null || userSession.getBuyer() == null || userSession.getId_user() == null) {
            System.out.println("Cannot update buyer info: userSession, token, buyer, or id_user is null");
            return;
        }

        try {
            Long id_user = userSession.getId_user();
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create("http://localhost:8080/users/" + id_user))
                    .header("Authorization", "Bearer " + userSession.getToken())
                    .GET()
                    .build();

            HttpResponse<String> response = httpClient.send(request, HttpResponse.BodyHandlers.ofString());
            System.out.println("Response status for /users/" + id_user + ": " + response.statusCode());
            System.out.println("Response body for /users/" + id_user + ": " + response.body());

            if (response.statusCode() == 200) {
                Map<String, Object> userData = gson.fromJson(response.body(), Map.class);
                String nickname = (String) userData.get("nickname");
                if (nickname == null) {
                    nickname = (String) userData.get("email");
                }
                if (nickname != null) {
                    userSession.getBuyer().setNickname(nickname);
                    profileButton.setText(nickname);
                    System.out.println("Updated profile button with nickname: " + nickname);
                } else {
                    System.out.println("Nickname not found in user data");
                }

                Number idBuyerNumber = (Number) userData.get("id_buyer");
                if (idBuyerNumber != null) {
                    Long id_buyer = idBuyerNumber.longValue();
                    System.out.println("Retrieved id_buyer: " + id_buyer);
                    userSession.getBuyer().setId_buyer(id_buyer);
                    System.out.println("id_buyer: " + (userSession.getBuyer() != null ? userSession.getBuyer().getId_buyer() : "null"));
                } else {
                    System.out.println("id_buyer not found in user data");
                }
            } else {
                System.out.println("Failed to fetch user info, status: " + response.statusCode());
            }
        } catch (Exception e) {
    ...
    private void openProductDetails(Product product) {
        app.switchSceneWithObject("/org/example/kursovoi/product-details-view.fxml", product);
    }
